...
class Xml
{
    public function __construct($data)
    {
        ...
        foreach ($data as $key => $timesheet)
        {
            ...
            if (!$last_date || $date == $last_date) {
                ...
                $content .= self::replacePlaceholders(array(
    ...
    static function weekdaysInGerman($day) {
        $day = trim($day);
    ...
    static function replacePlaceholders($data, $string)
    {
        if (is_array($data))
        {
    ...
    static function convertUnixToTime($timestamp){
